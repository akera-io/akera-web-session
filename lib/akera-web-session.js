    ...
    if (typeof config.store === "object" && config.store.connector) {
      try {
        ...
        config.store.on("disconnect", function (err) {
          if (err)
            akeraApp.log("warn", "Session store disconnected - " + err.message);
        });
      } catch (err) {
        akeraApp.log(
          "warn",
          'Unable to initialize session store "' +
            config.store.connector +
            '" - ' +
            err.message
        );
      }
    ...
    router.use(session(config), function (req, res, next) {
      if (req.session && req.session.get !== "function") {
        req.session.all = function () {
          if (config.isolated !== true) return this;
        ...
        req.session.get = function (name) {
          if (config.isolated !== true) return this[name];

          if (this.req.broker && this._data) {
            var broker = this._data[this.req.broker.alias];

            if (broker && typeof broker === "object") return broker[name];
          }
        ...
        req.session.set = function (name, val) {
          if (config.isolated !== true) {
            if (val === undefined) delete this[name];
            else this[name] = val;
            return;
          }
          ...
          if (this.req.broker) {
            ...
            if (val === undefined) {
              if (this._data[broker]) delete this._data[broker][name];
            } else {
              if (!this._data) this._data = {};

              if (!this._data[broker]) this._data[broker] = {};
